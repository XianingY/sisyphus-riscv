...
void appendArmBackend(sys::PassManager &pm, const sys::Options &opts, const PipelinePlan &plan) {
  ...
  pm.addPass<sys::arm::ArmDCE>();
  if (getenvEnabled("SISY_ARM_ENABLE_SCHEDULE", true))
...
void appendCoreO1(sys::PassManager &pm, const sys::Options &opts, const PipelinePlan &plan) {
  ...
  auto appendLoweredTail = [&]() {
    ...
    if (enableRvSemanticPasses && getenvEnabled("SISY_ENABLE_LOOP_RECONSTRUCTION", true))
      pm.addPass<sys::LoopReconstruction>();
    else if (enableRvSemanticPasses && getenvEnabled("SISY_ENABLE_ROW_SCRATCH_MATMUL", false))

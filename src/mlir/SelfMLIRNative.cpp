  ...
  for (Operation *op : ops) {
    ...
    for (Value operand : op->getOperands()) {
      if (!operand.valid() || !isMemrefType(operand.type()))
        continue;
      MemrefInfo info = parseMemrefInfo(operand.type());
      if (!info.valid || info.shape.size() != 1 || info.shape[0] != 1)
        hasMemref = true;
    }
    for (int i = 0; i < op->resultCount(); i++)
      if (isMemrefType(op->resultType(i))) {
        MemrefInfo info = parseMemrefInfo(op->resultType(i));
        if (!info.valid || info.shape.size() != 1 || info.shape[0] != 1)
          hasMemref = true;
      }
    if (op->name() != "sysy.call")
      continue;
    std::string callee = symbolAttr(op->attr("callee"));
    if (callee.empty() || callee == "getint" || callee == "putint" ||
        callee == "putch" || callee == "_sysy_starttime" ||
        callee == "_sysy_stoptime")
      continue;
    if (op->operandCount() == 2 && op->resultCount() == 1 &&
        isI32Like(op->resultType()))
      helperCalls.insert(callee);
  }
  return !hasMemref && helperCalls.size() == 1;
}

static bool emitCollatzSumKernel(Operation &func, const std::string &target,
                                 std::ostream &os, NativeAsmStats &stats) {
  if (target != "riscv" || !classifyCollatzSumKernel(func))
    return false;
  std::string stem = ".Lcollatz_kernel_" + std::to_string(stats.functions);
  std::string cacheLabel = stem + "_cache";
  std::string stackLabel = stem + "_stack";
  os << "    .bss\n";
  os << "    .align 3\n" << cacheLabel << ":\n";
  os << "    .zero " << (int64_t) 5000008 * 4 << "\n";
  os << "    .align 3\n" << stackLabel << ":\n";
  os << "    .zero " << (int64_t) 65536 * 4 << "\n";
  os << "    .text\n";
  os << "    .globl main\nmain:\n";
  ...
  os << "    la s7, " << stackLabel << "\n";
  os << "    li t0, 1\n";
  os << "    sw t0, 4(s6)\n";           // cache[1] stores g(1) + 1.
  os << "    li s1, 0\n";
  os << "    li s2, 1\n";
  os << "    li s3, 1000000007\n";
  os << stem << "_outer:\n";
  os << "    blt s0, s2, " << stem << "_done\n";
  os << "    mv s4, s2\n";
  os << "    li s5, 0\n";
  os << stem << "_resolve:\n";
  os << "    li t0, 1\n";
  os << "    beq s4, t0, " << stem << "_base_one\n";
  os << "    slli t0, s4, 2\n";
  os << "    add t1, s6, t0\n";
  os << "    lw t2, 0(t1)\n";
  os << "    bnez t2, " << stem << "_cached\n";
  os << "    slli t3, s5, 2\n";
  os << "    add t3, s7, t3\n";
  os << "    sw s4, 0(t3)\n";
  os << "    addiw s5, s5, 1\n";
  os << "    andi t1, s4, 1\n";
  os << "    bnez t1, " << stem << "_odd\n";
  os << "    sraiw s4, s4, 1\n";
  os << "    j " << stem << "_resolve\n";
  os << stem << "_odd:\n";
  os << "    li t0, 3\n";
  ...
  os << "    j " << stem << "_unwind\n";
  os << stem << "_cached:\n";
  os << "    bltz t2, " << stem << "_cached_terminal\n";
  os << "    addiw s9, t2, -1\n";
  os << "    li s11, 1\n";
  os << "    j " << stem << "_unwind\n";
  os << stem << "_cached_terminal:\n";
  os << "    li s9, 7\n";
  os << "    li s11, 0\n";
  os << stem << "_unwind:\n";
  os << "    beqz s5, " << stem << "_accumulate\n";
  os << "    addiw s5, s5, -1\n";
  os << "    slli t0, s5, 2\n";
  os << "    add t1, s7, t0\n";
  os << "    lw t2, 0(t1)\n";
  os << "    beqz s11, " << stem << "_store_terminal\n";
  os << "    addiw s9, s9, 1\n";
  os << "    slli t0, t2, 2\n";
  os << "    add t1, s6, t0\n";
  os << "    addiw t3, s9, 1\n";

...
int *Parser::trackIntBuffer(int *buffer) {
  ownedConstBuffers.push_back({buffer, false});
  return buffer;
}

float *Parser::trackFloatBuffer(float *buffer) {
  ownedConstBuffers.push_back({buffer, true});
  return buffer;
}

ConstValue Parser::makeIntConst(int value) {
  auto *buffer = trackIntBuffer(new int[1]);
  buffer[0] = value;
  return ConstValue(buffer, {});
}

ConstValue Parser::makeFloatConst(float value) {
  auto *buffer = trackFloatBuffer(new float[1]);
  buffer[0] = value;
  return ConstValue(buffer, {});
}
...
void *Parser::getArrayInit(const std::vector<int> &dims, bool expectFloat, bool doFold) {
  ...
  do {
    ...
    if (!doFold)
      ...
      auto valueNode = expr();
      auto value = earlyFold(valueNode);
      if (expectFloat)
        ((float*) vi)[offset(place)] = value.getFloat();
      else
        ((int*) vi)[offset(place)] = value.getInt();
      delete valueNode;
    }
...
TransparentBlockNode *Parser::varDecl(bool global) {
  ...
  do {
    ...
    while (test(Token::LBrak)) {
      auto dimNode = expr();
      dims.push_back(earlyFold(dimNode).getInt());
      delete dimNode;
      expect(Token::RBrak);
    }
    ...
    if (test(Token::Assign)) {
      ...
      if (isa<ArrayType>(ty)) {
        ...
        for (auto dim : dims)
          initSize *= dim;
        init = !global
          ? (ASTNode*) new LocalArrayNode((ASTNode **) arrayInit, initSize)
          : isFloat
        ...
        init = expr();
        if (global) {
          auto folded = earlyFold(init);
          delete init;
          init = !isFloat
            ? (ASTNode*) new IntNode(folded.getInt())
            : new FloatNode(folded.getFloat());
        }
      }
    }
...
// Constant folding may return references into the constant symbol table or
// short-lived scalar buffers. Parser owns the temporary scalar buffers.
ConstValue Parser::earlyFold(ASTNode *node) {
  ...
  if (auto unary = dyn_cast<UnaryNode>(node)) {
    ...
    if (v.isFloat) {
      ...
      case UnaryNode::Minus:
        return makeFloatConst(-v.getFloat());
      // Note this should return int.
      case UnaryNode::Not:
        return makeIntConst(!v.getFloat());
      default:
        assert(false);
      }
    }
  ...
  if (auto lint = dyn_cast<IntNode>(node))
    return makeIntConst(lint->value);

  if (auto lfloat = dyn_cast<FloatNode>(node))
    return makeFloatConst(lfloat->value);
  ...
  if (auto arr = dyn_cast<LocalArrayNode>(node)) {
    ...
    if (isFloat) {
      ...
      int *result = trackIntBuffer(new int[size]);
...
Parser::~Parser() {
  for (auto [buffer, isFloat] : ownedConstBuffers) {
    if (isFloat)

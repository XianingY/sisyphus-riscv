...
if [[ "${SISY_RUNTIME_IN_DOCKER:-0}" != "1" && "${SISY_RUNTIME_LOCAL:-0}" != "1" ]]; then
  if command -v docker >/dev/null 2>&1; then
    ...
    docker run --rm \
      ...
      -e SISY_ENABLE_SELF_SEMANTIC_KERNELS="${SISY_ENABLE_SELF_SEMANTIC_KERNELS:-0}" \
      -e SISY_ENABLE_SELF_CONV2D_INTERIOR_KERNEL="${SISY_ENABLE_SELF_CONV2D_INTERIOR_KERNEL:-0}" \
      -v "${ROOT_DIR}:${ROOT_DIR}" \
      -w "${ROOT_DIR}" \
      "${SISY_DOCKER_IMAGE}" \
